...
public class Asteroids extends Game {
	...
	private java.util.List<Bullet> removeAsteroids = new ArrayList<Bullet>();
       ...
       if(collision) {
		...
		for (Bullet bullet : bullets) {
			...
			if(bullet.outOfBounds()== true) {
				removeAsteroids.add(bullet);
			}
		}
		
		// Using this contains() method from Polygon, along with your getCenter() method from the Bullet class, 
		// determine if a bullet and asteroid have collided
		...
		for (int i = 0; i < bullets.size(); i++) {
			for (int j = 0; j < randomAsteroids.size(); j++) {
				if (randomAsteroids.get(j).contains(bullets.get(i).getCenter())){
					...
					removeAsteroids.add(bullets.get(i));
				}
			}
		}
		bullets.remove(removeAsteroids);
